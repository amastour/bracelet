...
class EloquentAuthenticationRepository implements AuthenticationContract {
	...
	public function login($request) {
		if ($this->auth->attempt($request->only('email', 'password'), $request->has('remember')))
		{
			...
			{
				$this->auth->logout();
				throw new GeneralException("Votre compte est actuellement désactivé.");
			}

			if ($this->auth->user()->status == 2)
			{
				$this->auth->logout();
				throw new GeneralException("Votre compte est actuellement interdit.");
			}
			...
			if ($this->auth->user()->confirmed == 0)
			{
				...
				$this->auth->logout();
				throw new GeneralException("Votre compte n'est pas confirmé. Veuillez cliquer sur le lien de confirmation dans votre boite e-mail, ou ".'<a href="'.route('account.confirm.resend', $user_id).'">cliquez içi</a>'." pour renvoyer un mail de confirmation.");
			}
		...
		throw new GeneralException('Email ou mot de passe n\'existe pas.');
	}
